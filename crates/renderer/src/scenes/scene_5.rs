//! Cornell box 風の部屋に roughness 0.3 の銀色 Conductor GGX 球を isotropic / anisotropic で 3 つ並べる。
...
pub fn create_scene_5() -> Result<(Scene, PinholeCamera), Box<dyn Error>> {
    ...
    let left_material = scene.add_material(Material::ConductorGgx(ConductorGgxMaterial::new(
        silver_base_color,
        0.3,
        -1.0,
    )));
    let center_material = scene.add_material(Material::ConductorGgx(ConductorGgxMaterial::new(
        silver_base_color,
        0.3,
        0.0,
    )));
    let right_material = scene.add_material(Material::ConductorGgx(ConductorGgxMaterial::new(
        silver_base_color,
        0.3,
        1.0,
    )));
