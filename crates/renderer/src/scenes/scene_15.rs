...
pub fn create_scene_15(
    ...
    let light_mesh = scene.add_mesh(load_gltf(Path::new("assets/models/light.glb"))?);
    scene.add_instance(light_mesh, light, Mat4::IDENTITY);

    let sphere_color_path = Path::new("assets/models/sphere-color.png");
    let sphere_roughness_path = Path::new("assets/models/sphere-roughness.png");
