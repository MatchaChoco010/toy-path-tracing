...
use crate::{
    ...
    mesh::{Bounds, load_obj, load_stl},
    scene::Scene,
};

pub fn create_scene_40() -> Result<(Scene, PinholeCamera), Box<dyn Error>> {
    ...
    let base_material = scene.add_material(Material::NormalizedLambert(
        NormalizedLambertMaterial::new(Vec3::splat(0.95)),
    ));
    let body_material = scene.add_material(Material::NormalizedLambert(
        NormalizedLambertMaterial::new(Vec3::new(0.95, 0.62, 0.45)),
    ));
