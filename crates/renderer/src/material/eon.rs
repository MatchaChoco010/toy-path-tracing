...
mod tests {
    ...
    fn sample_rejects_below_geometric_normal() {
        let material = EonMaterial::new(Vec3::ONE, 0.5);
        let mut vtx = test_shading_vertex(Vec2::ZERO);
        // Geometric normal points opposite to shading normal: every sample
        // should be rejected by the geometric-normal cosine test.
        vtx.ng = -Vec3::Z;
