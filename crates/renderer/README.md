...
現在の CLI では、出力先、出力画像サイズ、シーン番号、1 ピクセルあたりのサンプル数、最大パストレース深度、使用する integrator、ログフィルタを指定できます。integrator を省略した場合は、MIS を使う `mis` が既定で選ばれます。
...
| `--log-filter <LOG_FILTER>` | `RUST_LOG` または `info` | `tracing_subscriber::EnvFilter` 形式のログフィルタです。指定時は `RUST_LOG` より優先されます。 |
| `-h, --help` | なし | ヘルプを表示します。 |

## MaterialX サポート

MaterialX 1.39.4 仕様の Volume を除くサブセットを `crates/renderer/src/scene_loader/mtlx_loader/` 以下にロード機構として実装しています。`.mtlx` ファイルを読み込み、`<surfacematerial>` を `Material::Mtlx` バリアントとして本リポジトリの BSDF / EDF / Light tree 経路に統合できます。
