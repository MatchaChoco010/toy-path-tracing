...
pub fn create_scene_2() -> Result<(Scene, PinholeCamera), Box<dyn Error>> {
    ...
    let silver = scene.add_material(Material::Mirror(MirrorMaterial::new(Vec3::splat(0.92))));
    let gold = scene.add_material(Material::Mirror(MirrorMaterial::new(Vec3::new(
        1.00, 0.76, 0.34,
    ))));
    ...
    let bunny_scale = uniform_scale_for_height(&bunny, 1.30);
    ...
    let bunny_transform = Mat4::from_translation(Vec3::new(-0.78, 0.0, -0.45))
        * Mat4::from_quat(game_rotation_degrees(0.0, 24.0, 0.0))
        * Mat4::from_scale(Vec3::splat(bunny_scale))
        * Mat4::from_translation(-bunny_pivot);
    scene.add_instance(bunny_mesh_index, silver, bunny_transform);

    let sphere_transform = Mat4::from_translation(Vec3::new(0.85, 0.0, -0.55))
        * Mat4::from_scale(Vec3::splat(sphere_scale))
        * Mat4::from_translation(-sphere_pivot);
    scene.add_instance(sphere_mesh_index, gold, sphere_transform);

    let camera = PinholeCamera::new(
        Vec3::new(0.0, 2.10, 7.0),
        Vec3::new(0.0, 1.35, -0.15),
        Vec3::Y,
        38.0_f32.to_radians(),
    );
